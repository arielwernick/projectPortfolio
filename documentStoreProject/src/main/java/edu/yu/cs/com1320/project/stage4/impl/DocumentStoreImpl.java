...
public class DocumentStoreImpl implements DocumentStore {
    ...
    private boolean checkForByteSpace(Document toAdd){
        int counter = 0;
        for(URI uri: keyRing){
        ...
        if(toAdd.getDocumentTxt() != null){
            String text = toAdd.getDocumentTxt();
            counter += text.getBytes().length;
        }else if(toAdd.getDocumentBinaryData() != null){
            counter += toAdd.getDocumentBinaryData().length;
        }
